...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (this->size() == 0) {
    ...
    Node* curr = this->getHeadPtr();
    bool insertAtHead = true;
    while (curr) {
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  while (leftCurr && rightCurr) {
    if(leftCurr->data > rightCurr->data) {
      mergedCurr = new Node(rightCurr->data);
      mergedCurr->prev = mergedPrev;
      mergedPrev->next = mergedCurr;
      mergedPrev = mergedCurr;
      merged.size_ += 1;
      rightCurr = rightCurr->next;
    } else {
      mergedCurr = new Node(leftCurr->data);
      mergedCurr->prev = mergedPrev;
      mergedPrev->next = mergedCurr;
      mergedPrev = mergedCurr;
      merged.size_ += 1;
  ...
  Node* restCurr = !leftCurr ? rightCurr : leftCurr;
  while (restCurr) {
    mergedCurr = new Node(restCurr->data);
    mergedPrev->next = mergedCurr;
    mergedCurr->prev = mergedPrev;
    merged.size_ += 1;
    restCurr = restCurr->next;
    mergedPrev = mergedCurr;
  }
  merged.head_ = sentinel->next;
  merged.head_->prev = nullptr;
  merged.tail_ = mergedPrev;
  delete sentinel;
  sentinel = nullptr;
  return merged;
}
